...
namespace AWSSDK.Examples
{
    public class HighLevelTableExample : DynamoDbBaseExample
    {
        ...
        void Awake()
        {
            ...
            scanOperation.onClick.AddListener(PerformScanOperation);
            _client = Client;
        }
        ...
        private void PerformScanOperation()
        {
            ...
            {
                var asyncSearch = Context.ScanAsync<Book>(
                                new ScanCondition(
                                    "Id",
                                    Amazon.DynamoDBv2.DocumentModel.ScanOperator.GreaterThan,
                                    10));

                asyncSearch.GetRemainingAsync((response) =>
                {
                    var result = response.Result;
                    Debug.Log("scanned count = " + result.Count);
                });
            });
        }
